...
#define GOOD_BASE64_ENCODE_LEN(L) (((L) / 3 * 4 + (((L) % 3 == 0) ? 0 : 4)))

#define GOOD_BASE64_DECODE_LEN(L, C2, C1) ((((L)-4) / 4 * 3) + (((C2) == '=' ? 1 : ((C1) == '=' ? 2 : 3))))
...
ssize_t good_base64_encode(const uint8_t *src, size_t slen, char *dst, size_t dmaxlen)
{
    ...
    if (dst == NULL)
        return GOOD_BASE64_ENCODE_LEN(slen);
    ...
    */
    ...
    for (size_t n = 0; n < formal; n += 3)
    {
        ...
        dst[dlen++] = _good_base64_encode_table((mark & 0x00FC0000) >> 18);
        /*2: 00000000 101010'10 0101'0101 00001111 & 0003F000 = 100101 */
        dst[dlen++] = _good_base64_encode_table((mark & 0x0003F000) >> 12);
        /*3: 00000000 10101010 0101'0101 00'001111 & 00000FC0 = 0101 00 */
        dst[dlen++] = _good_base64_encode_table((mark & 0x00000FC0) >> 6);
        /*4: 00000000 10101010 01010101 00'001111' & 0000003F = 001111 */
        dst[dlen++] = _good_base64_encode_table((mark & 0X0000003F) >> 0);
    }
...
ssize_t good_base64_decode(const char *src, size_t slen, uint8_t *dst, size_t dmaxlen)
{
    ...
    if (dst == NULL)
        return GOOD_BASE64_DECODE_LEN(slen, src[slen - 2], src[slen - 1]);
    ...
    */
    ...
    for (long n = 0; n < formal; n += 4)
    {
        ...
        */
        ...
        /*2: a b */
        mark = (mark << 6 | _good_base64_decode_table(src[n + 1]));
        /*3: a b c */
        mark = (mark << 6 | _good_base64_decode_table(src[n + 2]));
        /*4: a b c d */
        mark = (mark << 6 | _good_base64_decode_table(src[n + 3]));
    ...
    */
    ...
    /*2:  */
    mark = (mark << 6 | _good_base64_decode_table(src[formal + 1]));
    /*3:   */
    if (src[formal + 2] != '=')
        mark = (mark << 6 | _good_base64_decode_table(src[formal + 2]));
    else
        mark = (mark << 6);
    //4:
    if (src[formal + 3] != '=')
        mark = (mark << 6 | _good_base64_decode_table(src[formal + 3]));
    else
        mark = (mark << 6);
