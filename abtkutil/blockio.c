...
ssize_t abtk_block_read(int fd, void *data, size_t size,abtk_buffer_t *buf)
{
    ...
    if (buf)
    {
        ...
        while (rsize < size)
        {
            ...
            {
                /*缓存有数据，先从缓存读取。*/
                rsize2 = abtk_buffer_read(buf, ABTK_PTR2PTR(void, data, rsize), size - rsize);
                if (rsize2 <= 0)
                    break;
            ...
            else if (buf->wsize == 0 || (size - rsize) < buf->size)
            {
                 ...
                 * 2：待读数据小于缓存空间。
                */
                rsize2 = abtk_buffer_import_atmost(buf, fd, buf->size);
                if (rsize2 <= 0)
                    break;
            }
            else
            {
                 ...
                 * 2：待读取数据大于缓存空间。
                */
                rsize2 = abtk_read(fd, ABTK_PTR2PTR(void, data, rsize), buf->size);
                if (rsize2 <= 0)
                    break;
...
ssize_t abtk_block_write(int fd, const void *data, size_t size,abtk_buffer_t *buf)
{
    ...
    if (buf)
    {
        ...
        while (wsize < size)
        {
            ...
            {
                /*缓存空间已满，先把缓存数据导出到文件。*/
                wsize2 = abtk_buffer_export_atmost(buf, fd, buf->size);
                if (wsize2 <= 0)
                    break;
            ...
            else if (buf->wsize > 0 || (size - wsize) < buf->size)
            {
                 ...
                 */
                wsize2 = abtk_buffer_write(buf, ABTK_PTR2PTR(void, data, wsize), size - wsize);
                if (wsize2 <= 0)
                    break;
            ...
            else
            {
                 ...
                 * 2：待写入数据大于缓存空间。
                */
                wsize2 = abtk_write(fd, ABTK_PTR2PTR(void, data, wsize), buf->size);
                if (wsize2 <= 0)
                    break;
