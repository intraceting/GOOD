...
int abtk_sqlite_backup(abtk_sqlite_backup_param *param)
{
    ...
    if (!backup_ctx)
        ABTK_ERRNO_AND_RETURN1(EINVAL, SQLITE_ERROR);
...
sqlite3* abtk_sqlite_open(const char *name)
{
    ...
    /*注册忙碌处理函数。*/
    sqlite3_busy_handler(ctx,abtk_sqlite_busy_melt,ctx);
...
int abtk_sqlite_journal_mode(sqlite3 *ctx, int mode)
{
    ...
    assert(ctx != NULL && mode >= ABTK_SQLITE_JOURNAL_OFF && mode <= ABTK_SQLITE_JOURNAL_WAL);

    switch (mode)
    {
    case ABTK_SQLITE_JOURNAL_OFF:
        sql = "PRAGMA journal_mode = OFF;";
        break;
    case ABTK_SQLITE_JOURNAL_DELETE:
        sql = "PRAGMA journal_mode = DELETE;";
        break;
    case ABTK_SQLITE_JOURNAL_TRUNCATE:
        sql = "PRAGMA journal_mode = TRUNCATE;";
        break;
    case ABTK_SQLITE_JOURNAL_PERSIST:
        sql = "PRAGMA journal_mode = PERSIST;";
        break;
    case ABTK_SQLITE_JOURNAL_MEMORY:
        sql = "PRAGMA journal_mode = MEMORY;";
        break;
    case ABTK_SQLITE_JOURNAL_WAL:
...
int abtk_sqlite_exec_direct(sqlite3 *ctx,const char *sql)
{
    ...
    assert(ctx != NULL && sql != NULL);
     ...
     stmt = abtk_sqlite_prepare(ctx,sql);
     if(!stmt)
        return -1;

     chk = abtk_sqlite_step(stmt);
