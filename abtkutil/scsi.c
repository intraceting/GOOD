...
const char* abtk_scsi_type2string(uint8_t type)
{
    switch(type)
    {
        case TYPE_DISK:
...
int abtk_scsi_sgioctl(int fd,struct sg_io_hdr *hdr)
{
...
int abtk_scsi_sgioctl2(int fd, int direction,
...
uint8_t abtk_scsi_sense_key(uint8_t *sense)
{
    assert(sense != NULL);
...
uint8_t abtk_scsi_sense_code(uint8_t *sense)
{
    assert(sense != NULL);
...
uint8_t abtk_scsi_sense_qualifier(uint8_t *sense)
{
    assert(sense != NULL);
...
int abtk_scsi_test(int fd, uint32_t timeout, abtk_scsi_io_stat *stat)
{
    uint8_t cdb[6] = {0};
...
int abtk_mtx_request_sense(int fd,uint32_t timeout, abtk_scsi_io_stat *stat)
{
    uint8_t cdb[6] = {0};
...
int abtk_scsi_inquiry(int fd,int vpd, uint8_t vid,
                      uint8_t *transfer, uint32_t transferlen,
                      uint32_t timeout,abtk_scsi_io_stat *stat)
{
    uint8_t cdb[6] = {0};
...
int abtk_scsi_inquiry_standard(int fd, uint8_t *type, char vendor[8], char product[16],
                               uint32_t timeout, abtk_scsi_io_stat *stat)
{
    uint8_t tmp[255] = {0};
    int chk;

    chk = abtk_scsi_inquiry(fd,0,0x00,tmp,255,timeout,stat);
    if(chk != 0)
        return -1;
    ...
    /* 去掉两端的空格。 */
    if(vendor)
        abtk_strtrim(vendor,isspace,2);
    if(product)
        abtk_strtrim(product,isspace,2);

    return 0;
}

int abtk_scsi_inquiry_serial(int fd, uint8_t *type, char serial[255],
                             uint32_t timeout, abtk_scsi_io_stat *stat)
{
    uint8_t tmp[255] = {0};
    int chk;

    chk = abtk_scsi_inquiry(fd,1,0x80,tmp,255,timeout,stat);
    if(chk != 0)
        return -1;
