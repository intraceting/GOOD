...
int abtk_notify_watch(int fd,abtk_notify_event_t *event,time_t timeout)
{
    ...
    /* 清除已经读取的。*/
    abtk_buffer_drain(event->buf);

    /*缓存无数据，先等待。*/
    if (event->buf->wsize <= 0)
    {
        /* 等待事件到来。 */
        if (abtk_poll(fd, 0x01, timeout) <= 0)
            return -1;

        /* 导入缓存。 */
        if( abtk_buffer_import_atmost(event->buf,fd,SIZE_MAX)<=0)
            return -1;
    }

    /*读事件，必须符合事件结构大小，不能多，也不能少。*/
    rlen = abtk_buffer_read(event->buf, &event->event, sizeof(struct inotify_event));
    ...
    rlen = abtk_buffer_read(event->buf,event->name,event->event.len);
