/*
 * This file is part of ABTK.
 * 
 * MIT License
 * 
 */
#include "odbc.h"

#ifdef __SQLEXT_H

SQLRETURN _abtk_odbc_check_return(SQLRETURN ret)
{
    SQLRETURN chk = SQL_ERROR;

    if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO)
        chk = SQL_SUCCESS;
    else
        chk = ret;

    return chk;
}

void _abtk_odbc_free_attr_destroy(abtk_allocator_t *alloc, void *opaque)
{
    /*只有这个单独申请的。*/
    void *p = alloc->pptrs[6];

    abtk_heap_freep(&p);
}

void abtk_odbc_free_attr(abtk_odbc_t *ctx)
{
    abtk_allocator_t *p;

    if (ctx->attr)
    {
        for (size_t i = 0; i < ctx->attr->numbers; i++)
        {
            p = (abtk_allocator_t *)ctx->attr->pptrs[i];

            abtk_allocator_unref((abtk_allocator_t**)&p);
        }    

        abtk_allocator_unref(&ctx->attr);
    }
}

SQLRETURN abtk_odbc_alloc_attr(abtk_odbc_t *ctx)
{
    SQLSMALLINT columns;
    abtk_allocator_t *p;
    SQLRETURN chk;

    assert(ctx != NULL);

    /*同一份数据集只需创建一次。*/
    if(ctx->attr != NULL)
        return SQL_SUCCESS;

    chk = SQLNumResultCols(ctx->stmt, &columns);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    ctx->attr = abtk_allocator_alloc(NULL, columns, 0);
    if (!ctx->attr)
    {
        chk = SQL_ERROR;
        goto final_error;
    }

    size_t sizes[7] = {NAME_MAX, sizeof(SQLSMALLINT), sizeof(SQLSMALLINT), sizeof(SQLULEN), sizeof(SQLSMALLINT), sizeof(SQLSMALLINT),0};

    for (size_t i = 0; i < ctx->attr->numbers; i++)
    {
        p = abtk_allocator_alloc(sizes, ABTK_ARRAY_SIZE(sizes), 0);
        if (!p)
        {
            chk = SQL_ERROR;
            goto final_error;
        } 

        ctx->attr->pptrs[i] = (uint8_t *)p;

        chk = SQLDescribeCol(ctx->stmt, (SQLSMALLINT)(i + 1), p->pptrs[0], p->sizes[0],
                             ABTK_PTR2PTR(SQLSMALLINT, p->pptrs[1], 0), ABTK_PTR2PTR(SQLSMALLINT, p->pptrs[2], 0),
                             ABTK_PTR2PTR(SQLULEN, p->pptrs[3], 0), ABTK_PTR2PTR(SQLSMALLINT, p->pptrs[4], 0),
                             ABTK_PTR2PTR(SQLSMALLINT, p->pptrs[5], 0));

        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        /*申请字段值的缓存区。*/
        p->pptrs[6] = abtk_heap_alloc(ABTK_PTR2OBJ(SQLULEN, p->pptrs[3], 0) + 1);
        if (!p->pptrs[6])
        {
            chk = SQL_ERROR;
            goto final_error;
        }

        /*记录可用长度。*/
        p->sizes[6] = ABTK_PTR2OBJ(SQLULEN, p->pptrs[3], 0) + 1;

        /*注册清理函数。*/
        abtk_allocator_atfree(p, _abtk_odbc_free_attr_destroy, NULL);
    }

    return SQL_SUCCESS;

final_error:

    abtk_odbc_free_attr(ctx);

    return chk;
}

SQLRETURN abtk_odbc_disconnect(abtk_odbc_t *ctx)
{
    SQLRETURN chk;

    assert(ctx != NULL);

    /*清理数据集属性。*/
    abtk_odbc_free_attr(ctx);
    
    if (ctx->stmt)
    {
        chk = SQLFreeHandle(SQL_HANDLE_STMT, ctx->stmt);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        ctx->stmt = NULL;
    }

    if (ctx->dbc)
    {
        chk = SQLDisconnect(ctx->dbc);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        chk = SQLFreeHandle(SQL_HANDLE_DBC, ctx->dbc);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        ctx->dbc = NULL;
    }

    if (ctx->env)
    {
        chk = SQLFreeHandle(SQL_HANDLE_ENV, ctx->env);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        ctx->env = NULL;
    }

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_connect(abtk_odbc_t *ctx, const char *uri, time_t timeout, const char *tracefile)
{
    SQLRETURN chk;

    assert(ctx != NULL && uri != NULL && timeout >= 0);

    if (!ctx->env)
    {
        chk = SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &ctx->env);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        chk = SQLSetEnvAttr(ctx->env, SQL_ATTR_ODBC_VERSION, (void *)SQL_OV_ODBC3, 0);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;
    }

    if (!ctx->dbc)
    {
        chk = SQLAllocHandle(SQL_HANDLE_DBC, ctx->env, &ctx->dbc);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        chk = SQLSetConnectAttr(ctx->dbc, SQL_ATTR_LOGIN_TIMEOUT, (SQLPOINTER)timeout, 0);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        if(tracefile)
        {
            chk = SQLSetConnectAttr(ctx->dbc, SQL_ATTR_TRACE, (SQLPOINTER)1, 0);
            if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
                goto final_error;

            chk = SQLSetConnectAttr(ctx->dbc, SQL_ATTR_TRACEFILE, (SQLPOINTER)tracefile, strlen(tracefile));
            if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
                goto final_error;
        }
    }

    chk = SQLDriverConnect(ctx->dbc, (SQLHWND)NULL, (SQLCHAR *)uri, SQL_NTS, NULL, 0, NULL, SQL_DRIVER_NOPROMPT);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_connect2(abtk_odbc_t *ctx, const char *product, const char *driver,
                             const char *host, uint16_t port, const char *db,
                             const char *user, const char *pwd,
                             time_t timeout, const char *tracefile)
{
    char uri[NAME_MAX]={0};

    assert(product != NULL && driver != NULL && host != NULL && port > 0 &&
           db != NULL && user != NULL && pwd != NULL);

    if (abtk_strcmp(product, "db2", 0) == 0)
    {
        snprintf(uri, NAME_MAX, "DRIVER=%s;HOSTNAME=%s;PORT=%hu;DATABASE=%s;UID=%s;PWD=%s;PROTOCOL=TCPIP;CONNECTTYPE=1;",
                 driver, host, port, db, user, pwd);
    }
    else if(abtk_strcmp(product,"oracle", 0) == 0)
    {
        snprintf(uri, NAME_MAX, "DRIVER={%s};DBQ=%s:%hu/%s;UID=%s;PWD=%s;",
                 driver, host, port, db, user, pwd);
    }
    else if(abtk_strcmp(product,"mysql", 0) == 0)
    {
        /*CHARSET=UTF8;*/
        snprintf(uri, NAME_MAX, "DRIVER=%s;SERVER=%s;TCP_PORT=%hu;DATABASE=%s;UID=%s;PWD=%s;",
                driver, host, port, db, user, pwd);
    }
    else if(abtk_strcmp(product,"sqlserver", 0) == 0)
    {
        snprintf(uri, NAME_MAX, "Driver={%s};Server=%s;TCP_PORT=%hu;Database=%s;UID=%s;PWD=%s;",
                driver, host, port, db, user, pwd);
    }
    else if(abtk_strcmp(product,"postgresql", 0) == 0)
    {
        /*CHARSET=UTF8;*/
        snprintf(uri, NAME_MAX, "DRIVER=%s;SERVER=%s;TCP_PORT=%hu;DATABASE=%s;UID=%s;PWD=%s;",
                driver, host, port, db, user, pwd);
    }

    return abtk_odbc_connect(ctx,uri,timeout,tracefile);
}

SQLRETURN abtk_odbc_autocommit(abtk_odbc_t *ctx, int enable)
{
    long flag;
    SQLRETURN chk;

    assert(ctx != NULL);

    flag = (enable ? SQL_AUTOCOMMIT_ON : SQL_AUTOCOMMIT_OFF);

    chk = SQLSetConnectAttr(ctx->dbc, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)flag, 0);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_tran_end(abtk_odbc_t *ctx, SQLSMALLINT type)
{
    SQLRETURN chk;

    assert(ctx != NULL);

    chk = SQLEndTran(SQL_HANDLE_DBC, ctx->dbc, type);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_prepare(abtk_odbc_t *ctx, const char *sql)
{
    SQLRETURN chk;

    assert(ctx != NULL && sql != NULL);

    /*清理旧的数据集属性。*/
    abtk_odbc_free_attr(ctx);

    if (!ctx->stmt)
    {
        chk = SQLAllocHandle(SQL_HANDLE_STMT, ctx->dbc, &ctx->stmt);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        chk = SQLSetStmtAttr(ctx->stmt, SQL_ATTR_CURSOR_TYPE, (SQLPOINTER)SQL_CURSOR_STATIC, 0);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;
    }

    chk = SQLPrepare(ctx->stmt, (SQLCHAR *)sql, SQL_NTS);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_execute(abtk_odbc_t *ctx)
{
    SQLRETURN chk;

    assert(ctx != NULL);

    chk = SQLExecute(ctx->stmt);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_finalize(abtk_odbc_t *ctx)
{
    SQLRETURN chk;

    assert(ctx != NULL);

    /*清理数据集属性。*/
    abtk_odbc_free_attr(ctx);
    
    if (ctx->stmt)
    {
        chk = SQLFreeHandle(SQL_HANDLE_STMT, ctx->stmt);
        if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
            goto final_error;

        ctx->stmt = NULL;
    }

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_exec_direct(abtk_odbc_t *ctx,const char *sql)
{
    SQLRETURN chk;

    chk = abtk_odbc_prepare(ctx,sql);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    chk = abtk_odbc_execute(ctx);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_affect(abtk_odbc_t *ctx, SQLLEN *rows)
{
    SQLRETURN chk;

    assert(ctx != NULL && rows != NULL);

    chk = SQLRowCount(ctx->stmt, rows);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_fetch(abtk_odbc_t *ctx, SQLSMALLINT direction, SQLLEN offset)
{
    SQLRETURN chk;

    assert(ctx != NULL && (direction >= 1 && direction <= 6));

    chk = SQLFetchScroll(ctx->stmt, direction, offset);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLRETURN abtk_odbc_get_data(abtk_odbc_t *ctx, SQLSMALLINT column, SQLSMALLINT type,
                             SQLPOINTER buf, SQLULEN max, SQLULEN *len)
{
    SQLLEN StrLen_or_Ind = 0;
    SQLLEN real_len = 0;
    abtk_allocator_t *p = NULL;
    SQLRETURN chk;

    assert(ctx != NULL && column >=0 && buf != NULL && max >0);

    /*创建数据集属性。*/
    chk = abtk_odbc_alloc_attr(ctx);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    p = (abtk_allocator_t *)ctx->attr->pptrs[column];
    if (!p)
    {
        chk = SQL_ERROR;
        goto final_error;
    }

    /*清除无效的值。*/
    memset(p->pptrs[6],0,p->sizes[6]);

    chk = SQLGetData(ctx->stmt, (SQLUSMALLINT)(column + 1), type, p->pptrs[6], p->sizes[6], &StrLen_or_Ind);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    if (StrLen_or_Ind > 0)
        real_len = ABTK_MIN(StrLen_or_Ind, max);
    else 
        real_len = ABTK_MIN(p->sizes[3], max);

    /*复制数据。*/
    memcpy(buf,p->pptrs[6],real_len);

    /*可能还需要返回长度。*/
    if(len)
        *len = real_len;

    return SQL_SUCCESS;

final_error:

    return chk;
}

SQLSMALLINT abtk_odbc_name2index(abtk_odbc_t *ctx, const char *name)
{
    SQLSMALLINT columns;
    abtk_allocator_t *p;
    SQLSMALLINT index;
    SQLRETURN chk;

    assert(ctx != NULL && name != NULL);

    /*创建数据集属性。*/
    chk = abtk_odbc_alloc_attr(ctx);
    if (_abtk_odbc_check_return(chk) != SQL_SUCCESS)
        goto final_error;

    for (index = ctx->attr->numbers - 1; index >= 0; index--)
    {
        p = (abtk_allocator_t *)ctx->attr->pptrs[index];
        if (!p)
        {
            chk = SQL_ERROR;
            goto final_error;
        }

        if (abtk_strcmp((char *)p->pptrs[0], name, 0) == 0)
            break;
    }

    return index;

final_error:

    return -1;
}

#endif //__SQLEXT_H
