...
static struct _good_mt_density_dict
{
    uint8_t type;
    const char *msg;
} good_mt_density_dict[] = {
    /* Copy from mt-st*/
    {0x00, "default"},
    {0x01, "NRZI (800 bpi)"},
    {0x02, "PE (1600 bpi)"},
...
const char *good_mt_density2string(uint8_t density)
{
    ...
    for (size_t i = 0; i < GOOD_ARRAY_SIZE(good_mt_density_dict); i++)
    {
        if (good_mt_density_dict[i].type != density)
            continue;

        msg_p = good_mt_density_dict[i].msg;
        break;
    }
